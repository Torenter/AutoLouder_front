        ...
        while True:
            if not self.result_queue.empty():
                ...
                if status.split('||')[0]=='0':
                    ...
                    ourUser.status = 'Error'#смена статуса
                    session.commit()# внесение изменений в БД
            else:
                continue
...
class Line(Base):
    ...
    def __init__(self, status, created,file_user_base,file_user_vars,file_user_vals,file_user_weight,user_id):
        self.status = status
    ...
    def __repr__(self):
        return "<Line('%s','%s','%s')>" % (self.status, self.created,self.file_user_base)
